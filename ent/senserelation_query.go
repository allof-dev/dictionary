...
type SenseRelationQuery struct {
	config
	ctx        *QueryContext
	order      []senserelation.OrderOption
	inters     []Interceptor
...
func (srq *SenseRelationQuery) QueryFrom() *SenseQuery {
	query := (&SenseClient{config: srq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(senserelation.Table, senserelation.FieldID, selector),
...
// First returns the first SenseRelation entity from the query.
// Returns a *NotFoundError when no SenseRelation was found.
func (srq *SenseRelationQuery) First(ctx context.Context) (*SenseRelation, error) {
	...
	if len(nodes) == 0 {
		return nil, &NotFoundError{senserelation.Label}
	}
	return nodes[0], nil
}
...
// FirstID returns the first SenseRelation ID from the query.
// Returns a *NotFoundError when no SenseRelation ID was found.
...
// Only returns a single SenseRelation entity found by the query, ensuring it only returns one.
// Returns a *NotSingularError when more than one SenseRelation entity is found.
// Returns a *NotFoundError when no SenseRelation entities are found.
func (srq *SenseRelationQuery) Only(ctx context.Context) (*SenseRelation, error) {
	...
	case 0:
		return nil, &NotFoundError{senserelation.Label}
	default:
		return nil, &NotSingularError{senserelation.Label}
	}
}
...
// OnlyID is like Only, but returns the only SenseRelation ID in the query.
// Returns a *NotSingularError when more than one SenseRelation ID is found.
...
func (srq *SenseRelationQuery) OnlyID(ctx context.Context) (id int, err error) {
	...
	case 0:
		err = &NotFoundError{senserelation.Label}
	default:
		err = &NotSingularError{senserelation.Label}
	}
	return
}
...
func (srq *SenseRelationQuery) All(ctx context.Context) ([]*SenseRelation, error) {
	...
	qr := querierAll[[]*SenseRelation, *SenseRelationQuery]()
	return withInterceptors[[]*SenseRelation](ctx, srq, qr, srq.inters)
}
...
func (srq *SenseRelationQuery) IDs(ctx context.Context) (ids []int, err error) {
	...
	if err = srq.Select(senserelation.FieldID).Scan(ctx, &ids); err != nil {
		return nil, err
	}
	return ids, nil
}
...
func (srq *SenseRelationQuery) Count(ctx context.Context) (int, error) {
	...
	return withInterceptors[int](ctx, srq, querierCount[*SenseRelationQuery](), srq.inters)
}
...
// Clone returns a duplicate of the SenseRelationQuery builder, including all associated steps. It can be
...
func (srq *SenseRelationQuery) Clone() *SenseRelationQuery {
	...
	return &SenseRelationQuery{
		config:     srq.config,
		ctx:        srq.ctx.Clone(),
		order:      append([]senserelation.OrderOption{}, srq.order...),
		inters:     append([]Interceptor{}, srq.inters...),
		predicates: append([]predicate.SenseRelation{}, srq.predicates...),
...
//	}
//
//	client.SenseRelation.Query().
//		GroupBy(senserelation.FieldRelType).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
func (srq *SenseRelationQuery) GroupBy(field string, fields ...string) *SenseRelationGroupBy {
	...
	grbuild := &SenseRelationGroupBy{build: srq}
	grbuild.flds = &srq.ctx.Fields
	grbuild.label = senserelation.Label
...
//
//	client.SenseRelation.Query().
//		Select(senserelation.FieldRelType).
//		Scan(ctx, &v)
func (srq *SenseRelationQuery) Select(fields ...string) *SenseRelationSelect {
	srq.ctx.Fields = append(srq.ctx.Fields, fields...)
	sbuild := &SenseRelationSelect{SenseRelationQuery: srq}
	sbuild.label = senserelation.Label
...
func (srq *SenseRelationQuery) prepareQuery(ctx context.Context) error {
...
func (srq *SenseRelationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*SenseRelation, error) {
	var (
		nodes       = []*SenseRelation{}
		withFKs     = srq.withFKs
		_spec       = srq.querySpec()
	...
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, senserelation.ForeignKeys...)
	}
	_spec.ScanValues = func(columns []string) ([]any, error) {
		return (*SenseRelation).scanValues(nil, columns)
	}
	_spec.Assign = func(columns []string, values []any) error {
		node := &SenseRelation{config: srq.config}
	...
	if query := srq.withFrom; query != nil {
		if err := srq.loadFrom(ctx, query, nodes, nil,
			func(n *SenseRelation, e *Sense) { n.Edges.From = e }); err != nil {
			return nil, err
		}
	}
	if query := srq.withTo; query != nil {
		if err := srq.loadTo(ctx, query, nodes, nil,
			func(n *SenseRelation, e *Sense) { n.Edges.To = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (srq *SenseRelationQuery) loadFrom(ctx context.Context, query *SenseQuery, nodes []*SenseRelation, init func(*SenseRelation), assign func(*SenseRelation, *Sense)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*SenseRelation)
	for i := range nodes {
		if nodes[i].sense_relation_from == nil {
			continue
		}
		fk := *nodes[i].sense_relation_from
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "sense_relation_from" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (srq *SenseRelationQuery) loadTo(ctx context.Context, query *SenseQuery, nodes []*SenseRelation, init func(*SenseRelation), assign func(*SenseRelation, *Sense)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*SenseRelation)
	for i := range nodes {
		if nodes[i].sense_relation_to == nil {
			continue
		}
		fk := *nodes[i].sense_relation_to
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "sense_relation_to" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (srq *SenseRelationQuery) sqlCount(ctx context.Context) (int, error) {
	_spec := srq.querySpec()
...
func (srq *SenseRelationQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(senserelation.Table, senserelation.Columns, sqlgraph.NewFieldSpec(senserelation.FieldID, field.TypeInt))
	_spec.From = srq.sql
	...
	if fields := srq.ctx.Fields; len(fields) > 0 {
		...
		for i := range fields {
			if fields[i] != senserelation.FieldID {
...
func (srq *SenseRelationQuery) sqlQuery(ctx context.Context) *sql.Selector {
	builder := sql.Dialect(srq.driver.Dialect())
	t1 := builder.Table(senserelation.Table)
	columns := srq.ctx.Fields
	if len(columns) == 0 {
		columns = senserelation.Columns
	}
...
func (srgb *SenseRelationGroupBy) Scan(ctx context.Context, v any) error {
	...
	return scanWithInterceptors[*SenseRelationQuery, *SenseRelationGroupBy](ctx, srgb.build, srgb, srgb.build.inters, v)
}

func (srgb *SenseRelationGroupBy) sqlScan(ctx context.Context, root *SenseRelationQuery, v any) error {
...
type SenseRelationSelect struct {
	*SenseRelationQuery
	selector
}
...
func (srs *SenseRelationSelect) Scan(ctx context.Context, v any) error {
	...
	return scanWithInterceptors[*SenseRelationQuery, *SenseRelationSelect](ctx, srs.SenseRelationQuery, srs, srs.inters, v)
}

func (srs *SenseRelationSelect) sqlScan(ctx context.Context, root *SenseRelationQuery, v any) error {
